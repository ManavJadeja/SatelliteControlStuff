function [accessBools, accessQuaternions] = OLDgetAccessQuaternions(root, scenario, satellite, facility, access, timeVector, dt)
...
% ACCESS QUATERNIONS
rows = length(accessRotationAngle)-1;

accessQuaternions(1:rows,1) = cosd(accessRotationAngle(1:rows)/2);
accessQuaternions(1:rows,2:4) = accessRotationAxis(1:rows,:).*sind(accessRotationAngle(1:rows)/2);
